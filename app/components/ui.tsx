...
export function CommentBox({ keyword, text, level = 'neutral' }: {
  ...
  return (
    <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
      {keyword && (
        <span style={{ fontSize: '1.3rem', fontWeight: 700, color, border: `1px solid ${color}`, borderRadius: 6, padding: '2px 8px', whiteSpace: 'nowrap' }}>
          {keyword}
        </span>
      )}
      <span style={{ fontSize: '1.3rem', color: 'var(--muted)', lineHeight: 1.6 }}>{text}</span>
    </div>
  )
}
