  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen(prev => !prev)} style={{
        ...
        color: open ? '#fff' : 'var(--text)', cursor: 'pointer', fontSize: '0.7rem',
      }}>
        🔖 북마크
      </button>
      ...
      {open && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.7rem', color: 'var(--muted)', marginBottom: 8 }}>북마크</div>

            {bookmarks.length === 0 ? (
              <div style={{ fontSize: '0.75rem', color: 'var(--muted)', textAlign: 'center', padding: '8px 0' }}>북마크가 없어요</div>
            ) : (
                ...
                {bookmarks.map(bm => (
                  <div key={bm.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', background: 'var(--surface2)', borderRadius: 6 }}>
                    <a href={bm.url} target="_blank" rel="noreferrer" onClick={() => setOpen(false)}
                      style={{ fontSize: '0.75rem', color: 'var(--text)', textDecoration: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
                      {bm.name}
                    </a>
                    <button onClick={() => removeBookmark(bm.id)}
                      style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '0.875rem', flexShrink: 0 }}>✕</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {showForm ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, borderTop: '1px solid var(--border)', paddingTop: 8 }}>
                <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="북마크 이름" autoFocus
                  style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: '0.75rem' }} />
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => { setShowForm(false); setForm({ name: '', url: '' }) }}
                    style={{ flex: 1, background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '5px', color: 'var(--muted)', cursor: 'pointer', fontSize: '0.7rem' }}>취소</button>
                  <button onClick={addBookmark}
                    style={{ flex: 1, background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 6, padding: '5px', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 700 }}>추가</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setShowForm(true)}
                style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px', color: 'var(--muted)', cursor: 'pointer', fontSize: '0.7rem' }}>
                + 북마크 추가
              </button>
            )}
          </div>
